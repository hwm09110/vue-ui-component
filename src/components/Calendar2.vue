  ...
  <div class="calendar-wrap">
    <div class="header">
      <div class="choosed-date">
        ...
        <span class="next-month" @click="handleNextMonth" v-show="canUseFutureMonth"></span>
      </div>
    </div>
    <div class="weeks">
    ...
    <div class="days">
      <div class="day-item prev-month-day" v-for="(dayItem) of prevMonthDays" :key="dayItem.key" @click="handleDayClick(dayItem)">
        <span v-show="showNoCurMonthDate" class="number">{{dayItem.day}}</span>
      </div>
      <div :class="['day-item', {'checked': dayItem.checked}, {'disabled': dayItem.disabled}, {'usableDate': dayItem.useable}]"  v-for="(dayItem) of curMonthDays" :key="dayItem.key" @click="handleDayClick(dayItem)">
        <span class="number">{{dayItem.day}}</span>
      </div>
      <div v-show="showNoCurMonthDate" class="day-item next-month-day" v-for="(dayItem) of nextMonthDays" :key="dayItem.key" @click="handleDayClick(dayItem)">
        <span class="number">{{dayItem.day}}</span>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    generateRangeDate(bdate, edate) {
      let oneDayMillisecond = 86400000 //一天毫秒数
      let diffMillisecond = new Date(bdate).getTime() - new Date(edate).getTime()
    ...
    generateUsableDate(referDate) {
      let oneDayMillisecond = 86400000 //一天毫秒数
      let prevDateMillisecond = new Date(referDate).getTime() - oneDayMillisecond
      let nextDateMillisecond = new Date(referDate).getTime() + oneDayMillisecond

      let prevDate = this.formatTimeStamp(prevDateMillisecond)
      let nextDate = nextDateMillisecond >= (new Date().getTime()) ? "" : this.formatTimeStamp(nextDateMillisecond) //排除当前日期之后的
    ...
    handleDayClick(dayItem) {
      ...
      let nowClickDateStr = `${ year }-${ this.formatNum(month) }-${ this.formatNum(day) }`
      ...
      if(dayItem.checked !== undefined && !dayItem.disabled){
        ...
        if(this.checkedDates.length >= this.dateLimit) {
          ...
          if(this.usableDates.length > 0 && !this.checkDateIsExist(nowClickDateStr, this.usableDates)){
            this.usableDates = []
            this.checkedDates = [nowClickDateStr]
            ...
            if(this.checkDateIsExist(nowClickDateStr, this.usableDates)){
              this.usableDates = this.usableDates.filter(usableDate => !this.isSameDate(usableDate, nowClickDateStr))
            }
          }
        }
    ...
    renderDays(year,month) {
      ...
      this.curMonthDays.forEach(item => {
        let curDate = `${ item.year }-${ this.formatNum(item.month) }-${ this.formatNum(item.day) }`
    ...
    .day-item{
      ...
      &.curDate{
        .number{
          display: inline-block;
          color: #fff;
          background:linear-gradient(311deg,rgba(71,174,254,1) 0%,rgba(71,209,254,1) 100%);
          font-weight: 500;
          width: 30px;
          height: 30px;
          line-height: 30px;
