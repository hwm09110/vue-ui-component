  ...
  methods: {
    ...
    renderMonths() {
      ...
      sourceMonthData = sourceMonthData.map((item) => {
        ...
        item.selected =
          item.year == nowYear && nowMonth == item.month ? true : false;
        item.cate = 2;
        return item;
      });
    ...
    scrollMonthList(year) {
      const topVal =
        (this.yearToListPosInfo[year] &&
          this.yearToListPosInfo[year]["startPos"]) ||
        0;
      ...
      this.$nextTick(() => {
        ...
        this.yearTipsPosInfo.forEach((itemPos, index) => {
          if (!this.yearToListPosInfo[itemPos.year]) {
            ...
            this.yearToListPosInfo[itemPos.year]["startPos"] =
              itemPos["offsetTop"];
            let nextYearTips = this.yearTipsPosInfo[index + 1];
            let nextYearTipsOffsetTop =
              (nextYearTips && nextYearTips["offsetTop"]) || 0;
        ...
        const scrollHandler = function(ev) {
          ...
          Object.keys(vm.yearToListPosInfo).forEach((key) => {
            if (
              scrollTopVal >= vm.yearToListPosInfo[key]["startPos"] &&
              scrollTopVal < vm.yearToListPosInfo[key]["endPos"]
            ) {
        ...
        this.monthListElem.addEventListener(
          "scroll",
